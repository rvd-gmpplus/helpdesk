...
- `prospect` → name `"Exploring"`, desc unchanged ("Not yet a customer, info request")
- `becoming` → name `"Onboarding"`, desc unchanged ("In application / onboarding")
...
## Section 4 — Pain points + Thoughts & feelings additions

In `LANES`:

- **`pains.self`:** append `{pain: "Customers struggle to understand what the scheme requirements actually mean for them"}`.
- **`feel.self`:** change `note` to `'"Where is this in the docs? How do I understand what it means?"'`. Keep `feel: "neg"` and `emoji: "😕"`.
...
Renderer: a new `MeetingOutcomes` component in `src/app.jsx`. Left card "Decisions taken" with green check icons; right card "Open questions" with orange question icons. Each item gets a status pill (`decision` = green, `open_question` = orange, `to_be_defined` = grey).
...
- **`styles.css`**
  - `.ch-tag.warn` — orange border for the rerouted-channel chip.
  - `.seg-disc` — circular icon for the new lane.
  - `.status-pill.decision` (green), `.status-pill.open_question` (orange), `.status-pill.to_be_defined` (grey), `.status-pill.exploring` (purple).
